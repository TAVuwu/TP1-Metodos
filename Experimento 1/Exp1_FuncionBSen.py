...
def main():
    intervalo=np.linspace(-1,1,50)      #precision del gráfico
    ...
    graficoInterp[0,0].set_title('Función original (en rojo) y (aproximación con polinomios de Lagrange)+1')
    graficoInterp[0,0].set_xlabel("x")
    graficoInterp[0,0].set_ylabel("y")
    graficoInterp[0,0].set_zlabel("z")
    ...
    for tamaño_grilla in range(3, 41):
        nodos_equiespaciados = np.linspace(-1, 1, tamaño_grilla)
        nodos_a_utilizar = [np.sin((np.pi/2)*x) for x in nodos_equiespaciados]
        if tamaño_grilla==20:
            fig,nodos=plt.subplots()
            for x in nodos_a_utilizar:
                nodos.scatter([x]*tamaño_grilla,nodos_a_utilizar,c="blue")
        ...
        for x in intervalo:      #fijar un valor de x
            ...
            if tamaño_grilla==8:
                ...
                for y in intervalo:
                    ...
                    zInterpolados_Xfija_Lagrange.append(poliLagrange_f_xFija(y)+1)
                for a in range(2):
                    for b in range(2):
        ...
        for y in intervalo:      #fijar un valor de y
            ...
            if tamaño_grilla==10:
                ...
                for x in intervalo:
                    ...
                    zInterpolados_Yfija_Lagrange.append(poliLagrange_f_yFija(x)+1)
                for a in range(2):
                    for b in range(2):
        ...
        graficoErrores[0,0].set_ylim(0,25)
        graficoErrores[0,1].set_ylim(0,0.7)
        graficoErrores[1,0].set_ylim(0,0.7)
        graficoErrores[1,1].set_ylim(0,0.7)

    graficoErrores[0,0].axhline(y=0.7, linestyle='--', color='gray',label="y = 0.7")
    graficoErrores[0,0].legend()
    plt.show()
