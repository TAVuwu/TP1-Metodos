...
def main():
    ...
    graficoInterp[1,1].set_title('Función original (en rojo) y (ablueproximación con splines cúbicos)+1')

    for tamaño_grilla in range(3, 41):
        nodos_equiespaciados = np.linspace(-1, 1, tamaño_grilla)
        nodos_a_utilizar = [(2*(np.log(x+2))/np.log(3))-1 for x in nodos_equiespaciados]
        if tamaño_grilla==20:
